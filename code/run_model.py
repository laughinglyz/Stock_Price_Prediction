...
def run_model(model,scaler, running_mode='train', train_set=None, valid_X=None, valid_Y=None, test_X=None, test_Y=None, 
	batch_size=8, learning_rate=1e-3, n_epochs=40, stop_thr=1e-6, shuffle=True):
	...
	if running_mode == 'train':
		...
		last_loss, loss_change = float('inf'),float('inf')
		while n_epochs and loss_change >= stop_thr:
			...
			last_loss = cur_loss
			n_epochs-=1
		return model, train_loss, valid_loss, valid_RMSE, valid_MAPE, valid_accuracy
	else:
...
def _test(model, test_X, test_Y, scaler, device=torch.device('cpu')):
	...
	with torch.no_grad():
		...
		directional_true[np.mean(test_X[:,-1,[1,2]],axis=-1)<=test_Y] = -1
		directional_prediction[np.mean(test_X[:,-1,[1,2]],axis=-1)<=outputs] = -1
